...
public enum TokenResultCode implements ResultCodeIfs{
    ...
    INVALID_TOKEN(HttpStatus.UNAUTHORIZED.value(), 4001, "유효하지 않은 토큰"),
    EXPIRED_TOKEN(HttpStatus.UNAUTHORIZED.value(), 4002, "만료된 토큰"),
    TOKEN_EXCEPTION(HttpStatus.UNAUTHORIZED.value(), 4003, "유효하지 않은 토큰 - 예외"),
    SUBJECT_EMPTY(HttpStatus.UNAUTHORIZED.value(), 4004, "SUBJECT IS NULL OR EMPTY"),
